...
class CreateFoodTableUseCase @Inject constructor(
    ...
    operator fun invoke(foodTable: FoodTable) = flow<ApiResponse<FoodTable>> {
        ...
        try {
            ...
            foodTableRepository.createFoodTable(request).collect {
                emit(it)
            }
        } catch (e: Exception) {
            e.printStackTrace()
            emit(ApiResponse.Failure(e.message ?: "Đã xảy ra lỗi khi tạo bàn ăn", 999))
        }
